...
test( 'Check all links - run with trace viewer to see results of checks', async ( { page } ) => {
  ...
  for (const p of Object.values(pages)) {
      // Await the function call for each iteration
      await checkLinks(p);
  }
} );

test( 'Check certain pages for  links - run with trace viewer to see results of checks', async ( { page } ) => {
  function isHttpUrl ( href: string ) {
    return href.startsWith( '/' ) || href.startsWith( 'http://' ) || href.startsWith( 'https://' );
  }
  async function checkLinks ( fileName: string ) {
